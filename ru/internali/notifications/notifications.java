...
public class notifications {
    ...
    public static void drawnotif(int i, String name, String text, Type type, int time) {
        ...
        GL11.glPushMatrix();
        if (time >= 110) {
            GL11.glTranslated((double)(sr.getScaledWidth() - 110), (double)((double)sr.getScaledHeight() - (double)time * 1.5 + 10.0), (double)0.0);
        } else {
            GL11.glTranslated((double)(sr.getScaledWidth() - time), (double)((double)sr.getScaledHeight() - (double)time * 1.5 + 10.0), (double)0.0);
        }
        ...
        if (alpha >= 0) {
            if (type == Type.Green) {
                ...
                Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(name, 10.0f, 2.0f, CatClient.getClientColor().getRGB());
